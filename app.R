  ...
  # Threshold setting panel =========================
     ...
     observeEvent(input$Selected_PrmOI_for_threshold, {
      ...
      plot_threshold_plot <- function(df, title){
        ...
        g = g +
          geom_vline(xintercept = reactV$thresholds[reactV$thresholds$parameter == xvar,"value"], linewidth = 1)+
          geom_hline(yintercept = reactV$thresholds[reactV$thresholds$parameter == yvar,"value"], linewidth = 0.5)+
          theme_bw() +
      ...
      observeEvent(input$run, {
       ...
       #convert combination string data frame to binary #######
        ...
        for( poi in POIs){
          ...
          ggsave(paste0(reactV$output_dir, "/tmp/", poi, ".pdf"), g, units = "mm", width = 29*length(unique(perCell_data$sample))*2, height = length(levels(perCell_data$Id))*8 , limitsize = FALSE ) 
        }
        ...
        plot_threshold_plot <- function(df, xvar){
          ...
          ggplot(df, aes_string(x = xvar, y = yvar) )+
            ...
            geom_vline(xintercept = thresholds[xvar], linewidth = 1)+
            scale_fill_gradientn(trans = "log10", colours=rev(rainbow(10, end = 4/6)))+
            theme_bw()+
            guides(fill="none")
        }
